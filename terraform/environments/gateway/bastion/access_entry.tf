...
resource "aws_eks_access_entry" "bastion_backend" {
  cluster_name  = data.terraform_remote_state.backend_eks.outputs.cluster_name
  principal_arn = aws_iam_role.bastion.arn
  type          = "STANDARD"
}

resource "aws_eks_access_policy_association" "bastion_backend" {
  cluster_name  = data.terraform_remote_state.backend_eks.outputs.cluster_name
  principal_arn = aws_iam_role.bastion.arn
  ...
  depends_on = [aws_eks_access_entry.bastion_backend]
}
